...
$main_content .= '<TR BGCOLOR="'.$bgcolor.'"><TD>Profession:</TD><TD>'.$vocation_name[$player->getWorld()][$player->getPromotion()][$player->getVocation()].'</TD></TR>';
			if(is_int($number_of_rows / 2)) { $bgcolor = $config['site']['darkborder']; } else { $bgcolor = $config['site']['lightborder']; } $number_of_rows++;
				...
				if(is_int($number_of_rows / 2)) { $bgcolor = $config['site']['darkborder']; } else { $bgcolor = $config['site']['lightborder']; } $number_of_rows++;
				$main_content .= '<TR BGCOLOR="'.$bgcolor.'"><TD>Balance:</TD><TD><font color=green > $ </font>'.$player->getBalance().' <b>G</b>old <b>C</b>oins. </TD></TR>';
			if(is_int($number_of_rows / 2)) { $bgcolor = $config['site']['darkborder']; } else { $bgcolor = $config['site']['lightborder']; } $number_of_rows++;
			...
			if(is_int($number_of_rows / 2)) { $bgcolor = $config['site']['darkborder']; } else { $bgcolor = $config['site']['lightborder']; } $number_of_rows++;
				...
				$marriage = $ots->createObject('Player');
            ...
            $itemList[(int)$v['id']] = ucwords(strtolower($v['name']));
				...
				$citizen = $player->getStorage(1500) && $player->getStorage(1501);
				$hunter = $player->getStorage(1523) && $player->getStorage(1523);
				$knight = $player->getStorage(1505) && $player->getStorage(1505);
				$mage = $player->getStorage(1502) && $player->getStorage(1502);
				$summoner = $player->getStorage(1508) && $player->getStorage(1509);
				$barbarian = $player->getStorage(1512) && $player->getStorage(1513);
				$druid = $player->getStorage(1514) && $player->getStorage(1514);
				$nobleman = $player->getStorage(1506) && $player->getStorage(1507);
				$oriental = $player->getStorage(1520) && $player->getStorage(1520);
				$warrior = $player->getStorage(1510) && $player->getStorage(1511);
				$wizard = $player->getStorage(1515) && $player->getStorage(1515);
				$assassin = $player->getStorage(1518) && $player->getStorage(1519);
				$beggar = $player->getStorage(1516) && $player->getStorage(1516);
				$pirate = $player->getStorage(1503) && $player->getStorage(1504);
				$shaman = $player->getStorage(1517) && $player->getStorage(1517);
				$norseman = $player->getStorage(1529) && $player->getStorage(1530);
				$jester = $player->getStorage(1521) && $player->getStorage(1522);
				$demonhunter = $player->getStorage(1525) && $player->getStorage(1526);
				$nightmare = $player->getStorage(1527) && $player->getStorage(1527);
				$brotherhood = $player->getStorage(1528) && $player->getStorage(1528);
				$yalaharian = $player->getStorage(1524) && $player->getStorage(1524);
				$warmaster = $player->getStorage(1533) && $player->getStorage(1533);
				$wayfarer = $player->getStorage(1534) && $player->getStorage(1534);
				...
				if(!$player->getHideChar()) 
				{
					...
					<br><br>
					<image src="http://outfit-images.ots.me/animatedOutfits1099/animoutfit.php?id='.$player->getLookType().'&addons='.$player->getLookAddons().'&head='.$player->getLookHead().'&body='.$player->getLookBody().'&legs='.$player->getLookLegs().'&feet='.$player->getLookFeet().'" width="64" height="64" style="width: 64px; height: 64px; position: absolute; background-position: 0 0; background-repeat: no-repeat; left: 110px; margin-top: -10px;"/>
					...
					<img style="margin-bottom: 10px;margin-left: 0px;margin-top: 3px;margin: 3px;width: 57px;height: 57px;background: rgba(155, 100, 155, 0.53);border-radius: 3px;border: 1.5px solid #434040;box-shadow: inset 0px 0px 3px rgba(0,0,0,0.5);" src="images/outfitbg.gif" width="64" height="64">
					</td>
					</tr>
					</span>';
				}
...
';
	...
	<table class="Table5" cellpadding="0" cellspacing="0">
		<tbody>
			...
			// Deaths list
			$deads = 0;
			$player_deaths = $SQL->query('SELECT `id`, `date`, `level` FROM `player_deaths` WHERE `player_id` = '.$player->getId().' ORDER BY `date` DESC LIMIT 10');
